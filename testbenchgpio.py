...
def test_motor_driver():
	return (
		GPIO.input(5) == GPIO.LOW,
		GPIO.input(6) == GPIO.LOW
	)
...
def get_data():
	...
	data['test_motor_current'] = round(motor_current(ADS1115_read_blocking(AIN1)), 2)
	data['test_powder_brake_current'] = round(pbrake_current(ADS1115_read_blocking(AIN2)), 2)
	data['test_psu_voltage'] = round(motor_rail(ADS1115_read_blocking(AIN3)), 2)
	return data
